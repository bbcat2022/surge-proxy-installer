...
certificate_tx_rollback_files() {
  snapshot_restore_file "${CERT_ACTIVE_DIR}/key.pem" "${CERT_SNAPSHOT_DIR}/key.pem" false || return 1
  snapshot_restore_file "${CERT_ACTIVE_DIR}/cert.pem" "${CERT_SNAPSHOT_DIR}/cert.pem" false
}
