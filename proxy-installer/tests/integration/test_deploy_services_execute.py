...
class DeployServicesExecuteTests(unittest.TestCase):
 def run_case(self, healthy=True):
  with tempfile.TemporaryDirectory() as t:
   ...
   for protocol in ('snell','anytls'):
    ...
    runtime_new.write_text(f'new-{protocol}'); runtime_old.write_text(f'old-{protocol}'); target.write_text(f'old-{protocol}')
    unit_new.write_text(f'new-{protocol}'); unit_old.write_text(f'old-{protocol}'); (units/unit_name).write_text(f'old-{protocol}')
...
'''
   ...
   values=[(runtime.read_text(),unit.read_text(),protocol) for runtime,unit,protocol in runtimes]
