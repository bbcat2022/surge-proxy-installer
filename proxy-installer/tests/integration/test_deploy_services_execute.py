...
class DeployServicesExecuteTests(unittest.TestCase):
 def run_case(self, healthy=True, existing=True, active='active', enabled='enabled', fail_action='', firewall_mode='', ufw_status='active', certificate=False):
  with tempfile.TemporaryDirectory() as t:
   root=Path(t); calls=root/'calls'; mock=root/'systemctl'
   mock.write_text(
    '#!/usr/bin/env bash\n'
    'printf "%s\\n" "$*" >> "$MOCK_CALLS"\n'
    'case "$1" in restart|start) [ -n "${MOCK_CERT_FILE:-}" ] && printf "%s:%s\\n" "$2" "$(cat "$MOCK_CERT_FILE")" >> "$MOCK_CERT_CALLS";; esac\n'
   ...
   certificate_env={}; certificate_active=None; certificate_calls=root/'certificate-calls'
   if certificate:
    candidate=root/'certificate-new'; candidate.mkdir()
    (candidate/'cert.pem').write_text('new-cert'); (candidate/'key.pem').write_text('new-key')
    certificate_active=root/'certificate-active'; certificate_active.mkdir()
    (certificate_active/'cert.pem').write_text('old-cert'); (certificate_active/'key.pem').write_text('old-key')
    openssl=root/'openssl'
    openssl.write_text('#!/usr/bin/env bash\ncase " $* " in *" -pubkey "*|*" -pubout "*) printf "matching-public-key\\n";; esac\nexit 0\n')
    openssl.chmod(0o700)
    certificate_env=dict(
     DEPLOY_CERTIFICATE_CANDIDATE_CERT=str(candidate/'cert.pem'),
     DEPLOY_CERTIFICATE_CANDIDATE_KEY=str(candidate/'key.pem'),
     DEPLOY_CERTIFICATE_ACTIVE_DIR=str(certificate_active),
     DEPLOY_CERTIFICATE_SNAPSHOT_DIR=str(root/'certificate-snapshot'),
     OPENSSL_BIN=str(openssl),MOCK_CERT_FILE=str(certificate_active/'cert.pem'),MOCK_CERT_CALLS=str(certificate_calls)
    )
...
'''
   result=subprocess.run(
    ...
    env=dict(os.environ,SYSTEMCTL_BIN=str(mock),MOCK_CALLS=str(calls),MOCK_ACTIVE=active,MOCK_ENABLED=enabled,MOCK_FAIL_ACTION=fail_action,**firewall_env,**certificate_env),
    check=False
   )
   ...
   certificate_result=(
    certificate_active.joinpath('cert.pem').read_text() if certificate_active and certificate_active.joinpath('cert.pem').exists() else None,
    certificate_active.joinpath('key.pem').read_text() if certificate_active and certificate_active.joinpath('key.pem').exists() else None,
    certificate_calls.read_text().splitlines() if certificate_calls.exists() else []
   )
   return result,values,exported.exists(),calls.read_text().splitlines() if calls.exists() else [],firewall_result,certificate_result
 def test_applies_all_services_and_exports_once(self):
  result,values,exported,_,_,_=self.run_case(True)
 ...
 def test_health_failure_restores_all_services(self):
  result,values,exported,_,_,_=self.run_case(False)
 ...
 def test_health_failure_removes_initial_service_materials(self):
  result,values,exported,calls,_,_=self.run_case(False,existing=False,active='inactive',enabled='not-found')
 ...
 def test_rollback_restores_inactive_disabled_state(self):
  result,values,exported,calls,_,_=self.run_case(False,active='inactive',enabled='disabled')
 ...
 def test_rollback_attempts_every_service_and_marks_restore_failure_dirty(self):
  result,values,exported,calls,_,_=self.run_case(False,active='inactive',enabled='disabled',fail_action='disable')
 ...
 def test_auto_firewall_is_part_of_service_transaction_and_dirty_if_later_health_fails(self):
  result,values,exported,_,firewall,_=self.run_case(False,firewall_mode='auto')
 ...
 def test_inactive_ufw_downgrades_auto_to_manual_without_rule_write(self):
  result,_,exported,_,firewall,_=self.run_case(True,firewall_mode='auto',ufw_status='inactive')
  context,calls=firewall
 ...
 def test_certificate_is_installed_with_services(self):
  result,_,exported,_,_,certificate=self.run_case(True,certificate=True)
  cert,key,calls=certificate
  self.assertEqual(result.returncode,0,result.stderr); self.assertTrue(exported)
  self.assertEqual((cert,key),('new-cert','new-key'))
  self.assertTrue(calls); self.assertTrue(all(call.endswith(':new-cert') for call in calls))
 def test_health_failure_restores_certificate_before_old_services_restart(self):
  result,_,exported,_,_,certificate=self.run_case(False,certificate=True)
  cert,key,calls=certificate
  self.assertNotEqual(result.returncode,0); self.assertIn('RESULT=rollback-success',result.stdout); self.assertFalse(exported)
  self.assertEqual((cert,key),('old-cert','old-key'))
  self.assertTrue(any(call.endswith(':new-cert') for call in calls))
  self.assertTrue(calls[-1].endswith(':old-cert'))
