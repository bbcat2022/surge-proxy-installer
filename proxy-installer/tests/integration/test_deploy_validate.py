...
class DeployValidateTests(unittest.TestCase):
 def test_validate_deploy_runs_full_local_gate_without_retaining_materials(self):
  with tempfile.TemporaryDirectory() as t:
   ...
   configured=subprocess.run(['bash',str(CLI),'--configure-snell','443','SnellPass88','domain','node.example.com'],env=env,text=True,capture_output=True,check=False)
  ...
  self.assertEqual(result.returncode,0,result.stderr); self.assertIn('binary-plan=snell:v5.0.1',result.stdout); self.assertIn('service-plan-count=1',result.stdout); self.assertIn('deploy-validation=passed',result.stdout)
