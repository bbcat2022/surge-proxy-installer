...
class DeployCertificatesTests(unittest.TestCase):
 def configured_environment(self, root):
  config=root/'config.yaml'; getent=root/'getent'; ss=root/'ss'
 ...
 def test_tls_domains_are_deduplicated_and_preflighted(self):
  with tempfile.TemporaryDirectory() as t:
   root=Path(t); _,env=self.configured_environment(root)
 ...
 def test_different_tls_domains_are_rejected_before_certificate_work(self):
  with tempfile.TemporaryDirectory() as t:
   root=Path(t); _,env=self.configured_environment(root)
   for command in (["--configure-anytls","8443","AnyTlsPass88","anytls.example.com","true","false"],["--configure-hysteria2","9000","Hy2Pass888","hy2.example.com","","10","false","","100"]):
    result=subprocess.run(['bash',str(CLI),*command],env=env,text=True,capture_output=True,check=False); self.assertEqual(result.returncode,0,result.stderr)
   result=subprocess.run(['bash',str(CLI),'--certificate-preflight','198.51.100.9'],env=env,text=True,capture_output=True,check=False)
  self.assertNotEqual(result.returncode,0)
  self.assertIn('必须使用同一个域名',result.stderr)
  self.assertNotIn('dns-domain=',result.stdout)
