...
class DeployBinariesTests(unittest.TestCase):
 def test_selects_first_pinned_candidate_and_installs_verified_binary(self):
  with tempfile.TemporaryDirectory() as t:
   ...
   manifests.joinpath('snell-amd64.txt').write_text(f'v1.2.3|2026-01-01|https://example.test/snell|{checksum}|raw|snell-server\n',encoding='utf-8')
...
'''
 ...
 def test_dry_run_does_not_create_work_directory(self):
  with tempfile.TemporaryDirectory() as t:
   root=Path(t); manifests=root/'manifests'; manifests.mkdir(); manifests.joinpath('snell-amd64.txt').write_text('v1.2.3|2026-01-01|https://example.test/snell|'+'a'*64+'|raw|snell-server\n',encoding='utf-8')
   work=root/'work'
